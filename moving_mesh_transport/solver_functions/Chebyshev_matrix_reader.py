...
class file_reader():
    def __init__(self, mass_matrix_name = 'moving_mesh_transport/matrix_files/MassMatrix.h5', J_matrix_name = 'moving_mesh_transport/matrix_files/JMatrix.h5', G_matrix_name = 'moving_mesh_transport/matrix_files/GMatrix.h5',
        ...
        self.J_mat_coeff_even = np.zeros((M, M, 2))
        self.J_mat_coeff_odd = np.zeros((M, M, 2))
        self.G_mat_coeff = np.zeros((M, M, 6))
        self.L_mat_coeff = np.zeros((M, M, 3))
        ...
        for ii in range(M):
            for jj in range(M):
                if ii > 0:
                    ...
                    self.L_mat_coeff[ii, jj] = f3['coefficients'][f'Element{ii+1}{jj+1}'][:].flatten()

                if (ii + jj + 2) % 2 == 0 and ii + jj != 0:
                    self.mass_mat_coeff_even[ii, jj] = f['coefficients'][f'Element{ii+1}{jj+1}'][:].flatten()
                    self.J_mat_coeff_even[ii, jj] = f1['coefficients'][f'Element{ii+1}{jj+1}'][:].flatten()
                else:
                    self.mass_mat_coeff_odd[ii, jj] = f['coefficients'][f'Element{ii+1}{jj+1}'][:].flatten()
                    self.J_mat_coeff_odd[ii, jj] = f1['coefficients'][f'Element{ii+1}{jj+1}'][:].flatten()
        for nn in range(N):
            for ii in range(M):
                for jj in range(M):
    ...
    def __call__(self):
        return  self.mass_mat_denom, self.J_mat_denom, self.G_mat_denom, self.L_mat_denom, self.VV_mat_denom, self.mass_mat_coeff_even, self.mass_mat_coeff_odd, self.J_mat_coeff_even, self.J_mat_coeff_odd, self.G_mat_coeff, self.L_mat_coeff, self.VV_mat_coeff_even, self.VV_mat_coeff_odd
