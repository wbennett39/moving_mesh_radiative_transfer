...
class sigma():
    def __init__(self, build):
        ...
        self.cs = np.zeros((self.N_space, self.Msigma))
    ...
    def integrate_quad(self, a, b, i, j, k):
        ...
        self.A[i,j,k] = fact * (b-a)/2 * np.sum(self.ws_quad *  Pn(i, argument, a, b) * Pn(j, argument, a, b) * Pn(k, argument, a, b))
    ...
    def create_integral_matrices(self):
        ...
        """
        self.AAA = np.zeros((self.M + 1,self.Msigma + 1,self.Msigma + 1))
    ...
    def make_vectors(self, edges, u, space):
        ...
        for i in range(self.M + 1):
            for j in range(self.Msigma + 1):
                for k in range(self.Msigma + 1):
                    self.VV[k] += cs[space, k] * u[j] * self.AAA[i, j, k] / dx
        return VV, VP
